...
- B. To remove any blank lines, add `| grep -v '^$'` to the end of your sequece of piped commands. (The `-v` option tells `grep` to return all lines *except* those that match the pattern, and `^$` is a regular expression that matches blank lines. We will learn more about regular expressions later in the course.)
