...
subtitle: "Modern Plain Text Social Science<br />Week 11"
...
As you know by now, `dplyr` is all about this.
...
The application step is "I want to fit a linear model to each piece". We did this with `nest()` last time.
...
The application step is "I want to fit a linear model to each piece" and get a summary. Again, we did this with `group_by()` and `nest()` last time.
...
![](../assets/10-parallel/10_amdahl_law.png)
...
![](../assets/10-parallel/10_idle_cpu)
...
Remember, processor clock cycles are _really fast_. They're measured in billions of cycles per second. We need to put those cores to work!
...
![](../assets/10-parallel/10_taxed_cpu)

---

![](../assets/10-parallel/10_north_atlantic.png)


---

![](../assets/10-parallel/10_global_mean.png)

---

![](../assets/10-parallel/10_all_seas.png)
