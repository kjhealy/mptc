...
subtitle: "Modern Plain Text Social Science<br />Week 05"
...
`r chunq_reveal("03a-dplyr-basics-43", smallcode = TRUE, widths = c(60,40), title = "Let's look at that again")`
...
- The thing inside the list with the "waving person", `\(x)`, is an _anonymous function_ or _lambda function_. It means "take whatever is passed to me, call it `x`, then immediately do whatever is written next".
...
In tidyverse functions, arguments that begin with a "`.`" generally have it in order to avoid confusion with existing items, or sometimes are "pronouns" referring to e.g. "the name of the thing we're currently talking about as we evaluate this function". The `.` doesn't have any special meaning beyond that.
...
```{r}
#| label: "03a-dplyr-basics-48a"
organdata |>
  group_by(consent_law) |>
  mutate(consent_mean = mean(donors, na.rm = TRUE)) |>
  select(country, year, donors, consent_law, consent_mean)
```

You might do this because you want to see how each country compares to the average for its consent law, or calculate a yearly difference from the consent law average, etc.
...
### Working in RStudio with Quarto documents
