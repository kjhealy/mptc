...
- Regular expressions are implemented across a very wide range of tools: shells of all kinds; 
...
- Regular expressions get their real power from special _wildcards_, i.e. tokens that match more than just literal strings, but also more general and more complex patterns.
...
## Searching for patterns {.incremental}
...
… how do you match a [literal]{.fg-orange} `\`?
...
egrep -r "Grep searches for text" . 
```
...
- Ripgrep, or `rg` is much quicker than `grep` and has some nice features
...
```{.zsh}
egrep "^The sky|night.$" files/examples/sentences.txt
```
