...
- [Apply]{.fg-red} a function, i.e. act on the data
...
- While loops are still lurking there underneath the surface, what we will do instead is to take advantage of the combination of functions and vectors (or tables) and apply or _map_ one to the other in order to generate results.
...
- So in addition to the basic [**`map()`**]{.fg-green}, which always returns a _list_, we also have [**`map_chr()`**]{.fg-green}, [**`map_int()`**]{.fg-green}, [**`map_dbl()`**]{.fg-green}, [**`map_lgl()`**]{.fg-green} and others.
- There are also variants for mapping over multiple inputs at once, like [**`map2()`**]{.fg-green}, for when we are passing two arugments to some function, and [**`pmap()`**]{.fg-green}, when we want to pass a list of any number of arguments.
...
R's vectorized rules add `b` to every element of `a`. The [**`+`**]{.fg-green} operation can be thought of as a function that takes each element of `a` and does something with it. In this case "add `b`". This is already a kind of iteration, in the sense that the operation recycles `b` down the vector `a` (unless they are the same length).
:::::
...
So how would we iterate this? What we want is to apply the [**`n_distinct()`**]{.fg-green} function to each column of `gapminder`, but in a way that still allows us to use pipelines and so on. Here we leave out the names so we can see that the result is literally the same function being applied to each column.
...
In reality you'd use [**across()**]{.fg-green}, like this:
...
That's a _lot_ nicer. Now it's just a tibble. We know those. This is only a small piece of what's in the summary object, but we'll get to the rest in a minute.
