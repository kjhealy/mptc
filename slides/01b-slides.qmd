---
title: "Text, Files, and the File System"
...
:::: {.columns}
::: {.column width="70%"}
![](../assets/01-file-system/01_ibm_system_360.png){width=90%}  
:::

::: {.column width="30%" .right}
No screens! Paper in, paper out for the operator; magnetic tapes for storage in the background. This is an IBM/360, the most widespread mainframe of the 1960s and early 1970s.
...
- The notion of "encoding information" is a very rich and complex topic that we sadly are going to skip.
...
- Text files, though, are sort of special. What's visibly in them corresponds much more closely to what they represent. A plain text file seems to represent the letter "A" with a symbol that looks like an "A". So much so that we can say it _is_ an "A".   
...
```{.zsh}
Sorry, Unix can't help you with this one
```
...
Unix is old. So is the idea of the command line. So are plain-text files. In an era of Generative AI and Large Language Models and all the rest, why are we covering this stuff?

Two related answers: 

- 1. Because Unix specifically is still everywhere and will be for a long time to come.

- 2. Because as soon as you try to do anything of any sort of complexity, depth, or even basic reliability with even the newest tools, you will almost certainly find yourself in a world governed by the metaphors and methods Unix originated. You will be in some sort of folder-based hierarchy consisting of plain-text files that are used to configure, launch, generate, or capture the output of executable applications by way of some sort of structured set of instructions written down as a series of commands. 

- The details of those instructions (the particular conventions they use to express what they mean) will vary depending on the application and the task at hand. But in essence you will always be doing the same thing. 
